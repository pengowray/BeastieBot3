    ...
    class BlurbFooter
    {
        ...
        public static string Footer(TaxonNode node, RedStatus status)
        {
            ...
            if (node.IsOrParentIs("Animalia"))
            {
                ...
                if (node.IsOrParentIs("CHORDATA"))
                { //TODO: work out if all IUCN CHORDATA are vertebrates
                    ...
                    else if (node.IsOrParentIs("Fish"))
                    { // added taxon
            ...
            if (!invertFound && node.IsOrParentIs("Invertebrate"))
            { // PseduoNode 'Invertebrate' might not be listed under Animalia
                footer.AppendLine("[[Category:Lists of invertebrates|" + thisList + "]]");
            }
            ...
            // Avoid pointless and sometimes deleterious bot edits
            //
            // BG19bot has been "deactivated" (last edit: 9 February 2017)
            // Yobot possibly still active. Last edit: 22 June 2024
            //
            //footer.AppendLine("{{bots|deny=BG19bot,Yobot}}"); 
            footer.AppendLine("{{bots|deny=Yobot}}");
            //
            // If not strong enough, try:
